...
def main():
    ...
    # 2. 建立資料庫與資料表
    conn = sqlite3.connect('pharmacies.db')  # 連線 SQLite 資料庫
    ...
    for city, count in med_count.items():
        print(f"INSERT INTO pharmacies VALUES ('{city}', {count}, '{now}')")  # 每次寫入時都會印出 SQL 指令（方便除錯）
        c.execute("INSERT INTO pharmacies VALUES (?, ?, ?)", (city, count, now))  # 使用問號來表示要插入的值，避免 SQL 注入攻擊
    conn.commit()
