...
def main():
    """主程式。"""
    print("此程式會要求您輸入一個大於 0 的正整數，然後計算它的階乘總和")  # 放在 main() 裡而非第一行程式，更具封裝性，程式被匯入其他模組時不會印出
    number = get_positive_integer()
    result = sum_of_factorial(number)
    print(f"1! + 2! + ... + {number}! = {result}")
